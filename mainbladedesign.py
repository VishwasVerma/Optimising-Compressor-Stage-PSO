...
def stator_diffusion_checker(alpha2):	# checks that flow exiting at stator is axial (Since this design is for last stage of LPC)
	alpha2= degree_2_radian(alpha2)
	D = 1 - np.cos(alpha2) + 0.5*9*np.sin(alpha2)* ( (np.cos(alpha2))**2 - 0.433)
	if D < 0.6 : return(1)		# diffusion factor at stator sholud be less than 0.6 for all sections
	else : return(0)
...
def blade_parameters_for_optimiser(Ca,Cw1,Cw2,T02,delta_T_o,n,AR,tip_beta1,tip_beta2):
	...
	solidity_factor = pitch_chord_relation (mean_V1,mean_V2)		# from emperical data (s/c)
...
MAX_EPOCHS = 10000	# Maximum Number of iteration allowed
POPULATION = population_size	# Initial Population Size

# Class specifing blade parameters at descriptive points
class blade_profile:
	...
	def __init__(self):
		...
		# parameters For PSO optimisation
		self.pBest = 0
	...
	def set_efficiency(self):
		...
		CDp = 0.018 # profile dr Assume NACA65-X10 aifoil parasite drag is given by this value 
	...
	def set_entropy_change(self):		# find entropy change across stage
...
# To find global best of the population
...
# to update population Using PSO
...
breaking_point = 50	# Store 100 potential candiates 
parameters = np.zeros((8,breaking_point))
# PSO algorthim
def PSO_algorthim():
	epoch = 0
	done = False
		...
		if epoch < MAX_EPOCHS:
			...
			for j in range(POPULATION):
				if (0.9-0.005) <= Blades[j].total_efficiency <= (0.9+0.005):
					...
					delta_T_o,tip_mach_number,n,AR = parameters[4][breaker],parameters[3][breaker],parameters[5][breaker],parameters[6][breaker]
					...
					parameters[7][breaker] = pop_generator(Ca,Cw1,Cw2,T02,delta_T_o,n,AR,filename,0)
...
def save_final_data(parameters,header,filename):
	...
	for j in range(1,(breaking_point+1)):
		...
		sheet1.write(j,3,parameters[3][j-1]) 	#tip_mach_number
		sheet1.write(j,4,parameters[4][j-1])	#dT
		sheet1.write(j,5,parameters[5][j-1])	#n
		sheet1.write(j,6,parameters[6][j-1])	#AR
		sheet1.write(j,7,parameters[7][j-1])	# number of blades
